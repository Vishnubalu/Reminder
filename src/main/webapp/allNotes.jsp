...
<%@page import="com.vishnu.web.model.NoteBooks"%>
...
if(from.equals("NOTEBOOKS")){
	 visible = "visible";
	bookName = (String) request.getSession().getAttribute("name");
}
else{
	 visible = "hidden";
}
...
<body >
	...
	<div class="container-fluid">
		...
		<ul class="nav navbar-nav navbar-right">
			<li><a id="logout" href="signup"><span
		...
		<ul class="nav navbar-nav navbar-right">
			...
			<a href="#" class="dropdown-toggle glyphicon glyphicon-bell notify"
				...
				style="color: white;">
			    <span class="badge">${reminderList.size()}</span>
			</a>
				...
				<ul class="dropdown-menu notify-drop">
					...
					<div class="drop-content">
						<li>
							<div id="notification" >
								<center>
								<c:forEach items="${reminderList}" var="notes">
								<h1><a
								<% session.setAttribute("note", notes); %>
								href="<c:url value="showNote.jsp?id=${notes.getNoteId()}"/>">${notes.getNoteName()}</a></h1>
								<p>(${notes.getReminderDate()})</p>
			...
			<li><a  id="addNewNote" href="noteconltroller" data-toggle="modal"
				data-target="#myModal" style="visibility: <%=visible%>">NEW NOTE</a></li>
		</ul>
	</div>
	<div class="modal fade" id="myModal" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
					...
					<form action="notecontroller?action=create" method="post" onsubmit="return validateNoteForm()">
...
<c:forEach items="<%=dailyList %>" var="note"> 
			<h3><a id="listOfNotes" href="showNote.jsp?id=${note.getNoteId()}">${note.getNoteName()}</a></h3>
...
<div class="column right">
	<div class="row">
		...
		<div class="col-md-8 text-left changes">
			<h1 style="color:blue;">
			<%
			if(from.equals("TODAY'S LIST")){
				out.println("TODAY'S LIST");
			}
			else if(from.equals("NOTEBOOKS")){
				out.println("NOTES FROM " + (String)bookName);
			}
			else{
				out.println("FROM " + user.getName());
			}
			%></h1>
		</div>
	</div><br>
								...
								<div class="panel-body">
									<div class="row">
										<div class="col-md-3 text-left">
											<a  id="notedetails" href="showNote.jsp?id=${note.getNoteId()}">${note.getNoteName()}</a>
										</div>
										<div class="col-md-3 text-left">Started on: <c:out value="${note.getStartDate()}" /></div>
										
										<c:choose>
											<c:when
												test="${note.getStatus() == 'complete'}">
												<div>
													<button type="button" id="status" class="btn btn-success btn-md medium">Complete</button>
												</div>
											</c:when>
											<c:when
												test="${note.getStatus() == 'inProgress'}">
												<div>
										...
										<div class="col-md-10 text-right">
											<a id="editNote"  href="editNote.jsp?id=${note.getNoteId()}"><span
												class="glyphicon glyphicon-edit" >Edit</span></a>
												
										</div>
										<div>
											<a href="notecontroller?action=delete&itemId=${note.getNoteId()}"><span
